...
const ProfilePage = () => {
  ...
  return (
    <div className="space-y-6 max-w-4xl mx-auto">
      <PageHeader
        badge={<Badge variant="purple">Identity Matrix</Badge>}
      ...
      <form onSubmit={handleSave} className="space-y-6">
        {/* Profile Card Header */}
        <GlassCard className="p-6 md:p-8 flex flex-col md:flex-row items-center gap-6 shadow-glass-glow border-indigo-500/20">
          ...
          <div className="flex-1 text-center md:text-left space-y-1.5">
            <div className="flex flex-wrap items-center justify-center md:justify-start gap-2">
              ...
              <span className="text-[10px] font-mono uppercase px-2.5 py-0.5 rounded-full bg-cyan-500/20 text-cyan-300 border border-cyan-500/30">
                {user?.role}
              </span>
            </div>
            <p className="text-xs text-slate-400 font-mono">{user?.email}</p>
            <p className="text-xs text-slate-300 pt-1 max-w-md">{bio || 'No status bio provided.'}</p>
          </div>
        </GlassCard>
        ...
        <GlassCard className="p-6 md:p-8 space-y-5">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              ...
              <input
                ...
                className="w-full px-3 py-2 rounded-xl bg-slate-900/60 border border-white/10 text-sm text-slate-200 focus:outline-none focus:border-cyan-500/50"
              />
            </div>

            <div>
              <label className="block text-xs font-medium text-slate-300 mb-1.5">Student / Faculty ID</label>
              <input
                type="text"
                value={studentId}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div>
              <label className="block text-xs font-medium text-slate-300 mb-1.5">Year / Level</label>
              <select
                value={year}
          ...
          <div>
            <label className="block text-xs font-medium text-slate-300 mb-1.5">Academic Bio</label>
            <textarea
              rows={3}
              value={bio}
